...
echo "[TASK 02] Install and configure ssh"
...
# Set Root password
echo "[TASK 03] Set root password"
echo -e "abc12345\nabc12345" | passwd root >/dev/null 2>&1 
echo "export TERM=xterm" >> /etc/bash.bashrc

echo "[TASK 04] Update /etc/hosts file"
cat >> /etc/hosts <<EOF
...
echo "[TASK 05] Install ucarp and haproxy"
apk update >/dev/null 2>&1 
apk add --no-cache ucarp haproxy >/dev/null 2>&1 

echo "[TASK 06] Configure ucarp"
ln -s /etc/init.d/ucarp /etc/init.d/ucarp.eth0
cat > /etc/ucarp/ucarp.pass <<EOF
abc12345
EOF
cat > /etc/conf.d/ucarp.eth0 <<EOF
UCARP_INTERFACE=eth0
UCARP_SOURCEADDRESS=192.168.100.101
UCARP_VIRTUALADDRESS=192.168.100.100
UCARP_VIRTUALPREFIX=24
UCARP_VHID=123
...
UCARP_OPTS="--shutdown"
EOF
chmod +x /etc/conf.d/ucarp.eth0
rc-update add /etc/conf.d/ucarp.eth0 >/dev/null 2>&1 

echo "[TASK 07] Configure haproxy"
...
EOF
rc-update add haproxy >/dev/null 2>&1